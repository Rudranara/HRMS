...
// Fetch holidays from the events table for Ahmedabad Office
$holiday_query = $conn->prepare("
    SELECT start_date FROM events WHERE YEAR(start_date) = ? AND MONTH(start_date) = ? AND office = 'Ahmedabad Office'
");
...
// Fetch total employees in Ahmedabad Office
$total_employees_query = $conn->prepare("
    SELECT COUNT(*) AS total_employees FROM employees WHERE office = 'Ahmedabad Office'
");
$total_employees_query->execute();
$total_employees = $total_employees_query->get_result()->fetch_assoc()['total_employees'];

// Fetch today's attendance for Ahmedabad Office
$today_date = date('Y-m-d');
$today_attendance_query = $conn->prepare("
    SELECT status FROM attendance WHERE DATE(punch_in_time) = ? AND office = 'Ahmedabad Office'
");
...
// Fetch attendance for the selected month and year for Ahmedabad Office
$stmt = $conn->prepare("
    SELECT * FROM attendance 
    WHERE YEAR(punch_in_time) = ? AND MONTH(punch_in_time) = ? AND office = 'Ahmedabad Office'
");
$stmt->bind_param("ii", $year, $month);
$stmt->execute();
$attendance = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Fetch pending leave requests for Ahmedabad Office
$pending_leave_query = $conn->prepare("
    SELECT COUNT(*) AS pending_leaves FROM leave_requests WHERE status = 'Pending' AND office = 'Ahmedabad Office'
");
$pending_leave_query->execute();
$total_pending_leaves = $pending_leave_query->get_result()->fetch_assoc()['pending_leaves'];

// Fetch total salary processed for the current month for Ahmedabad Office
$salary_query = $conn->prepare("
    SELECT SUM(net_salary) AS total_salary FROM salary 
    WHERE YEAR(created_at) = ? AND MONTH(created_at) = ? AND office = 'Ahmedabad Office'
");
...
// Fetch upcoming birthdays for Ahmedabad Office
$birthdays_query = $conn->prepare("
    SELECT name, DATE_FORMAT(dob, '%d %M') AS birthday FROM employees 
    WHERE MONTH(dob) = ? AND DAY(dob) >= DAY(NOW()) AND office = 'Ahmedabad Office'
");
...
?>
    ...
    <select name="site" id="site" class="form-control" onchange="redirectToSite()">
        ...
        <option value="odisha_dashboard" >Odisha Site</option>
        <option value="ahmedabad_dashboard" selected>Ahmedabad Site</option>
