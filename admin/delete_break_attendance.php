...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT selfie_in, selfie_out FROM break_attendance WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $delete_stmt = $conn->prepare("DELETE FROM break_attendance WHERE id = ?");
        $delete_stmt->bind_param("i", $attendance_id);
        if ($delete_stmt->execute()) {
            // Success: Redirect with a success message
            echo "break record and associated selfies deleted successfully!";
            header("Location: manage_break_attendance");
            exit();
        } else {
