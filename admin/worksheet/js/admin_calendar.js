document.addEventListener('DOMContentLoaded', function(){
  ...
  function render(){
    ...
    fetch(`/worksheet/public/api/holidays_month?m=${String(m).padStart(2,'0')}&y=${y}`).then(r=>r.json()).then(hol=>{
      const set = {};
  ...
  function toggleHoliday(date, title){
    fetch('/worksheet/public/api/holiday_add', { method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({date:date,title:title}) }).then(()=> render());
  }

  function removeHoliday(date){
    fetch('/worksheet/public/api/holiday_delete', { method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({date:date}) }).then(()=> render());
  }
});
