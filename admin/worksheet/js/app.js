...
document.addEventListener('DOMContentLoaded', function(){
	...
	if (holidayForm) {
		...
		const loadList = ()=>{
			...
			fetch(`/worksheet/public/api/holidays_month?m=${m}&y=${y}`).then(r=>r.json()).then(data=>{
				...
				list.querySelectorAll('.btn-del').forEach(b=> b.addEventListener('click', ()=>{
					...
					fetch('/worksheet/public/api/holiday_delete', { method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({date:d}) }).then(()=>loadList());
				}));
			});
		};
		loadList();
		document.getElementById('addHoliday').addEventListener('click', (e)=>{
			...
			fetch('/worksheet/public/api/holiday_add', { method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({date:d,title:t}) }).then(()=>{ loadList(); });
		});
	}
	...
	function loadUsers(){
		...
		fetch('/worksheet/public/api/users_list').then(r=>r.json()).then(data=>{
			...
			usersListEl.querySelectorAll('.btn-del').forEach(b=> b.addEventListener('click', ()=>{
				...
				fetch('/worksheet/public/api/user_delete', { method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({id: b.dataset.id}) }).then(()=> loadUsers());
			}));
		});
	}
	...
	if (document.getElementById('saveUserBtn')) document.getElementById('saveUserBtn').addEventListener('click', ()=>{
		...
		fetch('/worksheet/public/api/user_save',{ method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body }).then(()=>{ if (userModal) userModal.hide(); loadUsers(); });
	});
	if (document.getElementById('deleteUserBtn')) document.getElementById('deleteUserBtn').addEventListener('click', ()=>{
		...
		fetch('/worksheet/public/api/user_delete',{ method:'POST', headers:{'X-CSRF-Token': window.CSRF_TOKEN}, body: new URLSearchParams({id}) }).then(()=>{ if (userModal) userModal.hide(); loadUsers(); });
	});
	loadUsers();
	...
	if (exportBtn) {
		exportBtn.addEventListener('click', ()=>{
			...
			const [y,m] = val.split('-');
			window.location = `/worksheet/public/api/export_csv?m=${m}&y=${y}`;
		});
	}
});
