...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // load users
  fetch('/admin/worksheet/api/users_list').then(r=>r.json()).then(data=>{
  ...
  loadBtn.addEventListener('click', ()=>{
    ...
    if (window.ReportCalendar) {
      ReportCalendar.init({ employeeId: u, year: y, month: parseInt(m,10), rootId: 'report-calendar-root' });
    } else {
  ...
  summaryBtn.addEventListener('click', ()=>{
    const u = userSel.value; const [y,m] = monthInp.value.split('-'); if (!u) return alert('Pick user');
    fetch(`/admin/worksheet/api/worksheets_user_month?employee_id=${u}&m=${m}&y=${y}`).then(r=>r.json()).then(rows=>{
      fetch(`/admin/worksheet/api/holidays_month?m=${m}&y=${y}&employee_id=${u}`).then(r=>r.json()).then(hol=>{
