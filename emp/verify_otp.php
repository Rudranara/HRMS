...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $otp = $_POST['otp'];
    $employee_id = $_SESSION['otp_employee_id'];

    // Check OTP validity
    $stmt = $conn->prepare("SELECT * FROM emp_otp_codes WHERE employee_id = ? AND otp_code = ? AND expires_at > NOW() AND used = 0");
    $stmt->bind_param("is", $employee_id, $otp);
    $stmt->execute();
    ...
    if ($result->num_rows == 1) {
        ...
        // Mark OTP as used
        $stmt = $conn->prepare("UPDATE emp_otp_codes SET used = 1 WHERE id = ?");
